...
async function extractTransactionsFromPDF(pdfPath: string): Promise<PhonePeTransaction[]> {
  try {
...
function parseParticipantsCSV(csvPath: string): Promise<ParticipantData[]> {
  ...
  // Updated helper function to trim headers
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log('First participant data:', participants.length > 0 ? JSON.stringify(participants[0]) : 'No participants');
